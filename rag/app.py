...
def write_top_bar():
    ...
    with col1:
        header = f"An AI App powered by AWS!"
        st.write(
            f"<h3 class='main-header'>{header}</h3>", unsafe_allow_html=True)
    with col2:
...
st.markdown('---')
input = st.text_input("You are talking to an AI, ask any question.",
                      key="input", on_change=handle_input)
